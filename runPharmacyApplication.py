...
def countNumberOfCustomers (myConn, thePharmacyID):
    try:
        ...
        check_sql = "SELECT pharmacyID FROM Pharmacy WHERE pharmacyID = %s;"
...
def updateOrderStatus (myConn, currentYear):
    ...
    try:
        ...
        update_sql = "UPDATE OrderSupply SET status = status || ' AS OF ' || %s WHERE status != 'cnld';"
...
def main():
    ...
    for pid in [11, 17, 44, 66]:
        ...
        if result >= 0:
            ...
            print(f"Error: countNumberOfCustomers failed for pharmacyID {pid}.")
    ...
    for year in [1999, 2025, 2031]:
        ...
        if result >= 0:
            ...
            print(f"Error: updateOrderStatus failed for year {year} (invalid year).")
    ...
    for maxDel in [2, 4, 3, 1]:
        ...
        if result >= 0:
            ...
            print(f"Error: deleteSomeOrders failed for maxOrderDeletions value {maxDel}.")
